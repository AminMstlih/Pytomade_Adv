...
def run():
    ...
    while True:
        try:
            ...
            # 6. Check Open Positions (Hedging Logic)
            ...
            try:
                ...
                if final_positions:
                    ...
                    for pos_side in list(final_positions.keys()): # Iterate over a copy
                        logger.info(f"Attempting to close {pos_side} position...")
                        # Closing might fail if TP/SL already triggered, but try anyway
            ...
            logger.error(f"Unexpected error in main loop: {e}", exc_info=True) # Log full traceback
            logger.info("Continuing loop after error...")
            time.sleep(config.POLLING_INTERVAL_SECONDS) # Don't spin too fast on errors
